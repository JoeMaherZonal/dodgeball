...
class League
  ...
  def points()
    while true
      data = @viewer.point_creator()
      return if data['home_team'] == "" || data['away_team'] == "" || data['player'] == ""
      home_team = find_team(data['home_team'])
      if home_team.nil?
        @viewer.invalid_team('home', data['home_team'])
        return
      end
      away_team = find_team(data['away_team'])
      if away_team.nil?
        @viewer.invalid_team('home', data['away_team'])
        return
      end
      player = find_player(data['player'])
      if player.nil?
        @viewer.invalid_player(data['player'])
        return
      end

      matches = find_matches(home_team.id, away_team.id)
      @viewer.matched_matches_title()
        if matches.length != 0
          matches.each do |match|
            puts "\t#{match.id}\t\t#{match.home_team.name[0..12]}\t#{match.away_team.name[0..12]}\t#{match.home_team_score}:#{match.away_team_score}"
            end
            puts ""
            break
        else
          @viewer.teams_have_not_played(home_team.name, away_team.name)
          gets.chomp
          return
        end
    end
    match_id = @viewer.select_match_by_id
    match = Match.return_match_by_id(match_id, @runner)

    while true
      if match.class == Match
        Point.new( {"player_id" => player.id, "match_id" => match.id}, @runner).save
        @viewer.successfuly_added_point()
        gets.chomp
        return
      else
        return if match_id == 0
        @viewer.no_such_match_id()
        return
      end
    end
  end
  ...
  def lineups()
    info = @viewer.linups_get_info()
    return if !validate_players(info)
    players = get_players(info)
    players['team_id'] = players['player1'].team_id
    lineuphash = {'player1_id' => players['player1'].id,
      'player2_id' => players['player2'].id,
      'player3_id' => players['player3'].id,
      'player4_id' => players['player4'].id,
      'player5_id' => players['player5'].id,
      'player6_id' => players['player6'].id,
      'team_id' => players['team_id']
    }
    Lineup.new(lineuphash, @runner).save
    @viewer.successfuly_added_linup()


  end

  def validate_players(info)
    all_valid = true
    players = get_players(info)

    count = 0
    6.times do
      @viewer.player_not_valid(info["player#{count}"]) if players["player#{count}"].nil?
      all_valid = false if players["player#{count}"].nil?
      count += 1
    end
    return all_valid
  end

  def get_players(info)
    players = {}

    count = 0
    6.times do
    players["player#{count}"] = Player.find_by_name(info["player#{count}"], @runner)
    count += 1
    end
    return players
  end
  ...
  def find_team(team_name)
    team = Team.find_team_like(team_name, @runner)
    return team
  end

  def find_player(player_name)
    player = Player.find_by_name(player_name, @runner)
    return player
  end

  def find_matches(home_team, away_team)
    Match.return_match_with_ids(home_team, away_team, @runner)
  end
